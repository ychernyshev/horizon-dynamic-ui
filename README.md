...
Horizon Dynamic UI — це JavaScript фреймворк для створення динамічних, декларативних веб-інтерфейсів без потреби в бекенді чи інструментах збірки. Розроблений з урахуванням простоти розробки та архітектурної ясності, Horizon дозволяє налаштовувати сцени, макети та компоненти для побудови динамічних та реактивних сторінок використовуючи лише html, css та javascript, але без використання node.js. 

 Ідеологія фреймворку: Ви, немов би режисер — просто монтуйте, структуруйте та імпровізуйте на вашій власній сцені.

### Ключові характеристики:

- ⚙️ Без залежностей, без кроку збірки — працює безпосередньо у браузері
...
> Horizon Dynamic UI — це фреймворк, який змінює підхід до створення сайтів, у логіку котрих не закладена наявність бекенду, та шаблонів сайтів. Він не потребує збірки, обов'язкового використання бібліотек, і працює прямо в браузері — з чистим HTML, CSS та JavaScript.

---
## Особливості Horizon: Dynamic UI Framework
...
Цей спосіб дозволяє будувати сцени як головні компоненти, що мають чіткий інтерфейс:

- template — HTML-структура
- mount(config) — точка входу. Такі сцени є повноцінними вузлами SceneManager'а, і можуть включатись у Layout Registry Scheme.
...
```
Декларативний стиль обирається для сцен із логікою, повідомленнями, динамікою — тобто там, де шаблон служить інтерфейсом до конфігурації та підключення додаткової логіки - `messages`, `dispatcher` тощо.
...
```
Імперативний стиль рекомендований для статичних сцен із мінімальним HTML, таких, як Page 404.

### 📘 5 Життєвий цикл: `onMount()` у Horizon
...
### 8. Horizon UI — EntriesFlow (у початковому стані)
...
- Синергія між BootSequence, Dispatcher, RenderPolicy та AccessLevel
...
### Документація 
- [У планах] `RenderPolicy.md`: принципи життєздатності компонентів
- `FutureUpdatePlans.md`: стратегія, яка лежить попереду

### 9. Інтегрована система повідомлень
...
### Варіативність створення посилань та кнопок
...
### 10. Ідеальний для Hosting Anywhere
...
### 🎨 11. Розроблений для динамічних шаблонів
...
- Додаєш логіку через mount()/onMount()
- Результат — сайт виглядає як “живий”, навіть без API

### 12. Мета фреймворка

Розгортати динамічні шаблони без білду і node.js на тих платформах, які не дозволяють такі інструменти використовувати. 
...
- Демо-версії UI для презентацій продукту, який ще не готовий для робочої демонстрації
...
Далі можете налаштовувати через mount()/onMount() і використовувати результат, як сцену, так й, як компонент до сцени. Компонувати декілька компонентів для створення сцени безпосередньо з SceneManager.js. Передавати параметри. Додавати scoped стилі та скрипти, та використовувати ще багато-багато інших різноманітних фішок з наявних хелперів. Додавати свої, унікальні.

### Структурна логіка Horizon Dynamic UI у порівнянні із Vue чи React
```
...
- Зробити розробку шаблонів сайтів **декларативною**, а не імперативною
...
| Описати необхідну логіку    |  `mount()` чи `onMount()`
...
- Код читається як конфігурація, яку можливо розширити логікою  
...
- Ти можеш лишити все так, як є, без особливих налаштувань. Однак, твій шаблон все одно буде динамічним
...
Головна ідея фреймворку: _Оживи свою сцену! Не просто макет, а поведінка_. 